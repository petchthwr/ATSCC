...
if __name__ == '__main__':
    ...
    n_features = 10
    n_filters = 32
    kernel_sizes = correct_sizes([9, 19, 39])
    bottleneck_channels = 32

    # Create a random tensor of size (batch_size, n_features, sequence_length)
    input_tensor = torch.randn(batch_size, n_features, sequence_length)

    # Create an InceptionBlock instance
    ...
    # Pass the tensor through the InceptionBlock
    output_tensor = inception_block(input_tensor)

    # Print sizes
    print(f"Input size: {input_tensor.size()} (N, E, S format)")
    print(f"Output size: {output_tensor.size()} (N, E, S format)")
